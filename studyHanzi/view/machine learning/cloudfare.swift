...
class cloudfareService {
    ...
    public func sendLlmRequest(with param: cloudfareLanguageConfig, completion: @escaping (Result<String, Error>) -> Void) {
        guard let randomAccount = cloudfareConfig.llm_acc.randomElement() else {
            completion(.failure(CustomError.noAccountsAvailable))
            return
        }
        
        let urlString = "https://api.cloudflare.com/client/v4/accounts/\(randomAccount.accoundId)/ai/run/\(cloudfareConfig.model_name)"
